/**
 * @file nodeOperation.ts
 * @description 本文件主要提供了一系列用于操作脑图节点的函数，包括节点的添加、删除、移动、复制以及编辑等。这些函数主要用于脑图编辑器的核心交互功能。
 * 
 * 使用场景：
 * 1. 节点操作：允许用户动态编辑脑图，通过增加、删除节点来完善脑图内容。
 * 2. 节点结构变更：支持更改节点的层级关系，包括插入父节点、兄弟节点等。
 * 3. 可视化同步：所有的节点操作同时更新 DOM 结构和数据对象，确保界面与数据的一致性。
 * 
 * 设计考量：
 * 1. 封装性与模块化：每个操作均被封装为独立的函数，便于维护和扩展。
 * 2. 数据与视图同步：操作函数中会同时更新节点对应的 DOM 和底层数据模型，确保界面和数据保持一致。
 * 3. 性能优化：通过合理使用 DOM 操作方法和事件总线，减少不必要的渲染，提升性能。
 * 4. 可扩展性：考虑到脑图操作功能可能需要不断扩展，函数设计时预留了一定的灵活性，例如支持动态传入自定义节点信息。
 */
